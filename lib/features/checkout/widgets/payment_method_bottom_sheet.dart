...
import 'package:fasta_deliveries/features/payment/widgets/offline_payment_button.dart';
...
class _PaymentMethodBottomSheetState extends State<PaymentMethodBottomSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: GetBuilder<CheckoutController>(builder: (checkoutController) {
        ...
        final zonePaymentMethods = _zonePaymentMethods(checkoutController);
        final allowedPaymentMethods = zonePaymentMethods.isEmpty
            ? activePaymentMethods
            : activePaymentMethods.where((method) => zonePaymentMethods.contains(method.getWay)).toList();
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    ...
                    widget.isDigitalPaymentActive && notHideDigital && allowedPaymentMethods.isNotEmpty ? Container(
                      decoration: BoxDecoration(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          ListView.builder(
                            itemCount: allowedPaymentMethods.length,
                            shrinkWrap: true,
                            physics: const NeverScrollableScrollPhysics(),
                            padding: EdgeInsets.zero,
                            itemBuilder: (context, index){
                              bool isSelected = checkoutController.paymentMethodIndex == 2 && allowedPaymentMethods[index].getWay! == checkoutController.digitalPaymentName;

                              return paymentButtonView(
                                padding: EdgeInsets.only(bottom: index == allowedPaymentMethods.length - 1 ? 0 : Dimensions.paddingSizeSmall),
                                disablePayments: disablePayments,
                                onTap: disablePayments ? null : () {
                                  checkoutController.setPaymentMethod(2);
                                  checkoutController.changeDigitalPaymentName(allowedPaymentMethods[index].getWay!);
                                  if(showChangeAmount) {
                                    setState(() {
                                ...
                                title: allowedPaymentMethods[index].getWayTitle!,
                                isSelected: isSelected,
                                image: allowedPaymentMethods[index].getWayImageFullUrl,
                              );
                            },
                          ),
                        ],
  ...
  List<String> _zonePaymentMethods(CheckoutController checkoutController) {
    final address = AddressHelper.getUserAddressFromSharedPref();
    final storeZoneId = checkoutController.store?.zoneId;
    if (address?.zoneData == null || storeZoneId == null) {
      return [];
    }
    for (final zone in address!.zoneData!) {
      if (zone.id == storeZoneId) {
        return zone.paymentGateways ?? [];
      }
    }
    return [];
  }
