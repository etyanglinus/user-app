...
import 'package:fasta_deliveries/common/widgets/add_favourite_view.dart';
...
import 'package:fasta_deliveries/common/widgets/hover/on_hover.dart';
import 'package:fasta_deliveries/common/widgets/hover/text_hover.dart';
...
import 'package:fasta_deliveries/features/home/widgets/web/widgets/arrow_icon_button.dart';
...
import 'package:fasta_deliveries/features/home/widgets/web/widgets/download_qr_scanner_widget.dart';
...
class _WebTopOffersNearMeState extends State<WebTopOffersNearMe> {
  ...
  bool _showQrWidget = true;

  @override
  void initState() {
    scrollController.addListener(_checkScrollPosition);
    final controller = Get.find<SplashController>();
    if (controller.appDownloadSection == null) {
      controller.fetchAppDownloadSection();
    }
    super.initState();
  }
    ...
    return GetBuilder<StoreController>(builder: (storeController) {
      return GetBuilder<SplashController>(builder: (splashController) {
        return GetBuilder<HomeController>(builder: (homeController) {
          ...
          final Widget offerSection = Stack(children: [
            Container(
          ...
          // final bool showDownloadSection = ((links?.playstoreUrl?.isNotEmpty ?? false) || (links?.appleStoreUrl?.isNotEmpty ?? false));

          if (ResponsiveHelper.isMobile(context) || !_showQrWidget) {
            return offerSection;
          }
          ...
          return  Row(
            ...
            children: [
              Expanded(child: offerSection),
              if(_showQrWidget) const SizedBox(width: Dimensions.paddingSizeLarge),
              if(_showQrWidget) DownloadQrScannerWidget(
                qrData: "${AppConstants.webHostedUrl}?qr=true",
                googlePlayUrl: links?.playstoreUrl,
                appStoreUrl: links?.appleStoreUrl,
                title: splashController.appDownloadSection?.downloadUserAppTitle,
                onClose: () {
                  setState(() {
                    _showQrWidget = false;
                  });
