...
class PriceConverter {
  static String convertPrice(double? price, {double? discount, String? discountType, bool forDM = false, bool isFoodVariation = false, String? formatedStringPrice, bool forTaxi = false, bool forMenuWallet = false}) {
    ...
    String currencySymbol = selectedZoneCurrencyCode ?? Get.find<SplashController>().configModel!.currencySymbol!;
  ...
  static String? get selectedZoneCurrencyCode {
    final address = AddressHelper.getUserAddressFromSharedPref();
    if (address?.zoneId == null || address?.zoneData == null) {
      return null;
    }
    for (final zone in address!.zoneData!) {
      if (zone.id == address.zoneId && zone.currencyCode != null && zone.currencyCode!.isNotEmpty) {
        return zone.currencyCode;
      }
    }
    return null;
  }
